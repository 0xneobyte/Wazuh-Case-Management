...
router.get('/', [
  ...
  try {
    ...
    if (req.query.assignedTo && ['admin', 'senior_analyst'].includes(req.user.role)) {
...
], loadCase, checkResourceOwnership(), async (req, res) => {
  res.status(200).json({
    success: true,
    data: req.case
  });
...
router.post('/', authorize('admin', 'senior_analyst'), [
  body('title').trim().notEmpty().withMessage('Title is required'),
  body('description').trim().notEmpty().withMessage('Description is required'),
  body('priority').isIn(['P1', 'P2', 'P3']).withMessage('Priority must be P1, P2, or P3'),
  body('severity').isIn(['Critical', 'High', 'Medium', 'Low']).withMessage('Invalid severity'),
  body('category').isIn(['Malware', 'Intrusion', 'Policy Violation', 'Vulnerability', 'Other']).withMessage('Invalid category'),
  ...
  try {
    ...
    if (req.body.assignedTo) {
      ...
      await User.findByIdAndUpdate(
        req.body.assignedTo,
        { $inc: { 'performance.currentCaseLoad': 1, 'performance.totalCasesAssigned': 1 } }
      );
    }
...
], loadCase, checkResourceOwnership(), async (req, res, next) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
    ...
    if (req.body.status && req.body.status !== caseData.status) {
      ...
      if (['Resolved', 'Closed'].includes(req.body.status) && !['Resolved', 'Closed'].includes(caseData.status)) {
        ...
        if (caseData.assignedTo) {
          const resolutionTime = Math.floor((new Date() - caseData.createdAt) / (1000 * 60));
          await User.findById(caseData.assignedTo).then(user => {
            if (user) {
              user.updatePerformance(true, resolutionTime);
              user.performance.currentCaseLoad = Math.max(0, user.performance.currentCaseLoad - 1);
...
// @access  Private (Admin, Senior Analyst)
router.put('/:id/assign', authorize('admin', 'senior_analyst'), [
  param('id').isMongoId().withMessage('Invalid case ID'),
  body('assignedTo').isMongoId().withMessage('Invalid user ID'),
], loadCase, async (req, res, next) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
    ...
    if (caseData.assignedTo && caseData.assignedTo.toString() !== assignedTo) {
      await User.findByIdAndUpdate(
        caseData.assignedTo,
...
router.get('/stats/overview', async (req, res, next) => {
  try {
    ...
    const result = stats.length > 0 ? stats[0] : {
      ...
      closedCases: 0, p1Cases: 0, p2Cases: 0, p3Cases: 0, overdueCases: 0
    };
