...
router.get('/overview', async (req, res, next) => {
  try {
    ...
    const [caseStats, priorityStats, statusTrends, recentActivity] = await Promise.all([
      ...
      Case.find(matchQuery)
        ...
        .select('caseId title status priority updatedAt createdAt assignedTo')
    ]);

    const overview = caseStats.length > 0 ? caseStats[0] : {
      totalCases: 0, openCases: 0, inProgressCases: 0, resolvedCases: 0, 
      closedCases: 0, overdueCases: 0, avgResolutionTime: 0
    };

    // Convert avgResolutionTime from milliseconds to hours
    if (overview.avgResolutionTime) {
      overview.avgResolutionTime = Math.round(overview.avgResolutionTime / (1000 * 60 * 60 * 100)) / 100;
    }
...
router.get('/performance', authorize('admin', 'senior_analyst'), [
  ...
  try {
    ...
    let userMatch = { role: { $in: ['analyst', 'senior_analyst'] }, isActive: true };
    if (req.query.userId) {
      userMatch._id = mongoose.Types.ObjectId(req.query.userId);
    }
              ...
              $filter: {
                ...
                cond: {
                  $and: [
                    ...
                    { $not: { $in: ['$$this.status', ['Resolved', 'Closed']] } }
                  ]
                }
              }
            }
          ...
          avgResolutionTime: {
            $avg: {
              $map: {
                ...
                in: {
                  $divide: [
                    { $subtract: ['$$case.resolution.resolvedAt', '$$case.createdAt'] },
...
router.get('/trends', [
  ...
  try {
    ...
    const [caseCreationTrends, caseResolutionTrends, categoryTrends, severityTrends] = await Promise.all([
      ...
      Case.aggregate([
        ...
        {
          $group: {
            _id: { $dateToString: { format: dateFormats[groupBy], date: '$resolution.resolvedAt' } },
            resolved: { $sum: 1 },
            avgResolutionTime: {
              $avg: {
                $divide: [
      ...
      Case.aggregate([
        { $match: matchQuery },
        {
          $group: {
            _id: '$severity',
            count: { $sum: 1 },
            openCount: { $sum: { $cond: [{ $in: ['$status', ['Open', 'In Progress']] }, 1, 0] } },
...
router.get('/sla', [
  ...
  try {
    ...
    const slaMetrics = await Case.aggregate([
        ...
        $group: {
          ...
          avgResponseTime: {
            $avg: {
              $cond: [
                { $ne: ['$assignedAt', null] },
                { $divide: [{ $subtract: ['$assignedAt', '$createdAt'] }, 1000 * 60] },
                null
              ]
            }
          }
      ...
      },
      {
        $project: {
          ...
          complianceRate: {
            $multiply: [
              { $divide: [{ $subtract: ['$totalCases', '$breachedCases'] }, '$totalCases'] },
              100
            ]
          },
    ...
    const summary = overallSLA.length > 0 ? {
      totalCases: overallSLA[0].totalCases,
      compliantCases: overallSLA[0].compliantCases,
      complianceRate: (overallSLA[0].compliantCases / overallSLA[0].totalCases * 100).toFixed(2)
