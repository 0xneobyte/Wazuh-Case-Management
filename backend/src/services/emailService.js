...
class EmailService {
  ...
  async sendCaseAssignmentNotification(caseData) {
    ...
    const htmlContent = this.generateEmailHeader() + `
      ...
      <div style="background-color: #f3f4f6; padding: 15px; border-radius: 6px; margin: 20px 0;">
        ...
        <p style="margin: 5px 0;"><strong>Case ID:</strong> ${caseData.caseId}</p>
        <p style="margin: 5px 0;"><strong>Title:</strong> ${caseData.title}</p>
        <p style="margin: 5px 0;"><strong>Priority:</strong> 
          <span style="background-color: ${priorityColors[caseData.priority] || '#6b7280'}; color: white; padding: 2px 8px; border-radius: 12px; font-size: 12px; font-weight: bold;">
            ${caseData.priority}
          </span>
        </p>
        <p style="margin: 5px 0;"><strong>Severity:</strong> ${severityIcons[caseData.severity] || '⚪'} ${caseData.severity}</p>
        <p style="margin: 5px 0;"><strong>Category:</strong> ${caseData.category}</p>
        <p style="margin: 5px 0;"><strong>Due Date:</strong> ${new Date(caseData.sla.dueDate).toLocaleString()}</p>
      </div>
      ...
      <div style="background-color: #eff6ff; padding: 15px; border-radius: 6px; margin: 20px 0;">
        ...
        <p style="margin: 5px 0; font-size: 14px;"><strong>Rule:</strong> ${caseData.wazuhAlert.ruleName || 'Unknown'}</p>
        <p style="margin: 5px 0; font-size: 14px;"><strong>Agent:</strong> ${caseData.wazuhAlert.agentName || 'Unknown'}</p>
        ${caseData.wazuhAlert.sourceIp ? `<p style="margin: 5px 0; font-size: 14px;"><strong>Source IP:</strong> ${caseData.wazuhAlert.sourceIp}</p>` : ''}
        <p style="margin: 5px 0; font-size: 14px;"><strong>Timestamp:</strong> ${new Date(caseData.wazuhAlert.timestamp).toLocaleString()}</p>
      </div>
      ` : ''}
    ...
    return await this.sendEmail(caseData.assignedTo.email, subject, htmlContent);
  }
  ...
  async sendCaseStatusUpdateNotification(caseData, previousStatus, updatedBy) {
    ...
    if (caseData.assignedBy && caseData.assignedBy.email && 
    ...
    const htmlContent = this.generateEmailHeader() + `
      ...
      <div style="background-color: #f3f4f6; padding: 15px; border-radius: 6px; margin: 20px 0;">
        ...
        <p style="margin: 5px 0;"><strong>Previous Status:</strong> 
          <span style="background-color: ${statusColors[previousStatus] || '#6b7280'}; color: white; padding: 2px 8px; border-radius: 12px; font-size: 12px;">
            ${previousStatus}
          </span>
        </p>
        <p style="margin: 5px 0;"><strong>New Status:</strong> 
          <span style="background-color: ${statusColors[caseData.status] || '#6b7280'}; color: white; padding: 2px 8px; border-radius: 12px; font-size: 12px;">
            ${caseData.status}
          </span>
        </p>
        <p style="margin: 5px 0;"><strong>Updated By:</strong> ${updatedBy.firstName} ${updatedBy.lastName}</p>
  ...
  async sendCaseClosureNotification(caseData, resolvedBy) {
    ...
    const resolutionTime = caseData.resolution?.resolvedAt ? 
      Math.round((new Date(caseData.resolution.resolvedAt) - new Date(caseData.createdAt)) / (1000 * 60 * 60)) : 0;
    ...
    const htmlContent = this.generateEmailHeader() + `
      ...
      <div style="background-color: #f3f4f6; padding: 15px; border-radius: 6px; margin: 20px 0;">
        ...
        <p style="margin: 5px 0;"><strong>Case ID:</strong> ${caseData.caseId}</p>
        <p style="margin: 5px 0;"><strong>Title:</strong> ${caseData.title}</p>
        <p style="margin: 5px 0;"><strong>Priority:</strong> ${caseData.priority}</p>
        <p style="margin: 5px 0;"><strong>Category:</strong> ${caseData.category}</p>
        <p style="margin: 5px 0;"><strong>Resolved By:</strong> ${resolvedBy.firstName} ${resolvedBy.lastName}</p>
        <p style="margin: 5px 0;"><strong>Resolution Time:</strong> ${resolutionTime} hours</p>
        <p style="margin: 5px 0;"><strong>Created:</strong> ${new Date(caseData.createdAt).toLocaleString()}</p>
  ...
  async sendSLAEscalationAlert(caseData, escalatedTo) {
    ...
    const hoursOverdue = Math.round((new Date() - new Date(caseData.sla.dueDate)) / (1000 * 60 * 60));
    ...
    const htmlContent = this.generateEmailHeader() + `
      ...
      <div style="background-color: #f3f4f6; padding: 15px; border-radius: 6px; margin: 20px 0;">
        ...
        <p style="margin: 5px 0;"><strong>Current Status:</strong> ${caseData.status}</p>
        <p style="margin: 5px 0;"><strong>Assigned To:</strong> ${caseData.assignedTo?.firstName || 'Unassigned'} ${caseData.assignedTo?.lastName || ''}</p>
        <p style="margin: 5px 0;"><strong>Due Date:</strong> ${new Date(caseData.sla.dueDate).toLocaleString()}</p>
        <p style="margin: 5px 0;"><strong>Created:</strong> ${new Date(caseData.createdAt).toLocaleString()}</p>
      </div>
      ...
      <div style="background-color: #f3f4f6; padding: 20px; border-radius: 6px; margin: 20px 0;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
          <div style="text-align: center;">
            <div style="font-size: 28px; font-weight: bold; color: #2563eb;">${stats.openCases || 0}</div>
            <div style="font-size: 14px; color: #6b7280;">Open Cases</div>
          </div>
          <div style="text-align: center;">
            <div style="font-size: 28px; font-weight: bold; color: #f59e0b;">${stats.inProgressCases || 0}</div>
  ...
  async sendTestEmail(toEmail, fromUser) {
    ...
    const htmlContent = this.generateEmailHeader() + `
      ...
      <div style="background-color: #f3f4f6; padding: 15px; border-radius: 6px; margin: 20px 0;">
        ...
        <p style="margin: 5px 0;"><strong>Sent By:</strong> ${fromUser.firstName} ${fromUser.lastName}</p>
