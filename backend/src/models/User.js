...
const userSchema = new mongoose.Schema({
  ...
  specializations: [{
    type: String,
    enum: ['Malware Analysis', 'Network Security', 'Incident Response', 'Forensics', 'Threat Hunting', 'Compliance']
  }],
  // Security settings
  security: {
    failedLoginAttempts: {
...
userSchema.methods.updatePerformance = function(caseResolved = false, resolutionTime = null) {
  if (caseResolved) {
    this.performance.totalCasesResolved += 1;
    
    if (resolutionTime) {
      const totalTime = this.performance.avgResolutionTime * (this.performance.totalCasesResolved - 1);
      this.performance.avgResolutionTime = (totalTime + resolutionTime) / this.performance.totalCasesResolved;
    }
  }
  ...
  if (caseResolved) {
    ...
    if (resolutionTime) {
      const totalTime = monthlyRecord.avgResolutionTime * (monthlyRecord.casesResolved - 1);
      monthlyRecord.avgResolutionTime = (totalTime + resolutionTime) / monthlyRecord.casesResolved;
    }
  }
};
